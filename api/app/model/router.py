from io import BytesIO
import os
from typing import List

from app import db
from app import settings 
from app import utils
from app.auth.jwt import get_current_user
from app.model.schema import PredictRequest, PredictResponse
from app.model.services import model_predict
from fastapi import APIRouter, Depends, File, HTTPException, UploadFile, status
from sqlalchemy.orm import Session
import shutil


router = APIRouter(tags=["Model"], prefix="/model")


@router.post("/predict")
async def predict(file: UploadFile, current_user=Depends(get_current_user)):
    rpse = {"success": False, "prediction": None, "score": None}
    # To correctly implement this endpoint you should:
    #   1. Check a file was sent and that file is an image, see `allowed_file()` from `utils.py`.
    #   2. Store the image to disk, calculate hash (see `get_file_hash()` from `utils.py`) before
    #      to avoid re-writing an image already uploaded.
    #   3. Send the file to be processed by the `model` service, see `model_predict()` from `services.py`.
    #   4. Update and return `rpse` dict with the corresponding values
    # If user sends an invalid request (e.g. no file provided) this endpoint
    # should return `rpse` dict with default values HTTP 400 Bad Request code
    # TODO

    # Step 1: Check if a file is sent and it's a valid image
    if not file:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No file uploaded.",
        )

    if not utils.allowed_file(file.filename):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="File type is not supported.",
        )

    # Step 2: Store the image on disk and calculate hash
    #upload_dir = "uploads"
    os.makedirs(settings.UPLOAD_FOLDER, exist_ok=True)  # Ensure the directory exists

    # Save the file temporarily

    # Create a BytesIO object from the BinaryIO stream
    #file_content = BytesIO(file.file.read())  # Reading the content of file.file and wrapping it in BytesIO

    # Calculate the file hash by passing the BytesIO stream
    file_hash = await utils.get_file_hash(file)    

    dst_filepath = os.path.join(settings.UPLOAD_FOLDER, file_hash)
    with open(dst_filepath, "wb") as buffer:
        shutil.copyfileobj(file.file, buffer)

    # Step 3: Process the file with the model service
    try:
        prediction, score = await model_predict(file_hash)
        rpse["success"] = True
        rpse["prediction"] = prediction
        rpse["score"] = score
        rpse["image_file_name"] = f"{file_hash}"
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error during prediction: {str(e)}",
        )

    # Step 4: Return the response
    return rpse