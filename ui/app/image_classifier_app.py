...
def predict(token: str, uploaded_file: Image) -> requests.Response:
    ...
    # Step 1: Prepare the file data as a tuple with the file name and content
    files = {
        "file": (uploaded_file.name, uploaded_file.getvalue())
    }
    
    # Step 2: Add the token to the headers
    headers = {
        "Authorization": f"Bearer {token}",  # Assuming the token is a Bearer token
    }

    # Step 3: Make a POST request to the predict endpoint
    url = f"{API_BASE_URL}/model/predict"  # Replace with your API's endpoint
    try:
        response = requests.post(url, files=files, headers=headers)

        # Log if the request was successful
        if response.status_code == 200:
            print("Prediction request was successful.")
        else:
            print(f"Prediction request failed: {response.status_code}, {response.text}")

    except requests.exceptions.RequestException as e:
        print(f"An error occurred while making the prediction request: {e}")
        response = None
...
def send_feedback(
    ...
    # Step 1: Create a dictionary with the feedback data
    feedback_data = {
        "feedback": feedback,
        "score": score,
    ...
    # Step 3: Make a POST request to the feedback endpoint
    url = f"{API_BASE_URL}/feedback"  # Replace with your actual feedback endpoint URL
    try:
...
st.markdown(
    "<h1 style='text-align: center; color: #4B89DC;'>Image Classifier12</h1>",
